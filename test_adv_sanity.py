...
class RarlSanityTests(unittest.TestCase):
    def test_control_env_behaves_like_rarl_env(self):
        # do control
        env = make_vec_env(lambda: HopperBulletEnv(render=False), seed=seed)
        ...
        bridge = Bridge()
        env = make_vec_env(
            lambda: ProtagonistRarlEnv(AdversarialHopperEnv(renders=False, adv_percentage=1.0), bridge),
            seed=seed
        )
        env = VecNormalize(env, norm_obs=True, norm_reward=True, clip_obs=10.)
        agent = PPO('MlpPolicy', env, verbose=True, seed=seed, n_steps=ts)
    ...
    def test_control_env_doesnt_behave_like_rarl_env(self):
        ...
        adv_env = make_vec_env(
            lambda: AdversarialRarlEnv(AdversarialHopperEnv(renders=False, adv_percentage=1.0), bridge),
            seed=seed
        )
